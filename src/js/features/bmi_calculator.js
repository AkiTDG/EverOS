...
function parseWeight(value) {
  const input = value.trim().toLowerCase();
  if (input.endsWith("kg")) return parseFloat(input);
  if (input.endsWith("lbs") || input.endsWith("lb")) return parseFloat(input) * 0.453592;
  const val = parseFloat(input);
  if (!isNaN(val)) return val;
  return NaN;
}
...
function saveBMIRecordBrowser(name, heightInput, weightInput, bmiStatus) {
...
export function BMICalculator(input, writeToConsole) {
  switch (bmiState.step) {
    ...
    case 1:
      bmiState.name = input;
      writeToConsole("Enter your height (e.g. 175cm, 5ft 7in, 1.75m):");
      bmiState.step = 2;
      break;
    case 2:
      ...
      writeToConsole("Enter your weight (e.g. 70kg, 154lbs):");
      bmiState.step = 3;
      break;
    case 3:
    ...
    case 4:
      if (input.toLowerCase() === 'y') {
        saveBMIRecordBrowser(bmiState.name, bmiState.heightInput, bmiState.weightInput, bmiState.result);
